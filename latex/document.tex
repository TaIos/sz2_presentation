...
\begin{frame}{Intervention 1/2}
	...
	\begin{enumerate}
		\item select some units and one concept
		\item force units on/off
		\item measure See if concept appears/disappears
	\end{enumerate}

	\begin{figure}
		\includegraphics[height=0.23\textheight]{../media/eq3}
	\end{figure}
\end{frame}

\begin{frame}{Intervention 2/2 — Finding sets of units with high ACE}

	\begin{itemize}
		\item Average causal effect (ACE): $\delta_{U \rightarrow c} \equiv \mathbb{E}_{z, P}[s_c(x_i)] - \mathbb{E}_{z, P}[s_c(x_a)]$
		\item ineffective, as there are $\binom{d}{|U|}$ subsets
		\item instead optimize $\alpha \in [0, 1]^d$
	\end{itemize}
	\begin{figure}
	\includegraphics[height=0.23\textheight]{../media/eq5}
\end{figure}
